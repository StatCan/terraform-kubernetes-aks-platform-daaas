...
resource "null_resource" "add_https_to_ingress_gateway" {
  provisioner "local-exec" {
    command = "kubectl -n istio-system patch gateway istio-autogenerated-k8s-ingress --patch '[{\"op\": \"add\", \"path\": \"/spec/servers/0/tls\", \"value\": {\"httpsRedirect\": true}}]'"
  }

  provisioner "local-exec" {
    command = "kubectl -n istio-system patch gateway istio-autogenerated-k8s-ingress --patch '[{\"op\": \"replace\", \"path\": \"/spec/servers/1/tls\", \"value\": {\"credentialName\": \"wildcard-tls\", \"mode\": \"SIMPLE\", \"privateKey\": \"sds\", \"serverCertificate\": \"sds\"}}]'"
  }
